...
** Log
   ...
   * Normally, before running executables: check the "checksum"
   ...
   * GUI appears (~Rgui.exe~)
   ...
   #+begin_src
    ...
    str(x) 
    plot(x)
    q()     # don't save the workspace image
   #+end_src

   * Saving the workspace image stores ~.RData~, ~.Rhistory~ files
   ...
   * Start R (enter R)
   * Test R with some commands
   * Issues: check the [[https://cran.r-project.org/faqs.html][R FAQ]] first
* NEXT 2.1 (01/19/22)
** Emacs+ESS
   ...
   | Set PATH            |                    |
   | Start R shell       | M-x R              |
   | Add init file       | .emacs sample file |
   | Create test.org     |                    |
   | Create R code block |                    |
   | Run R code block    |                    |
