...
* Writing functions - w6s13 - (02/16/22)
** News
...
** Overview
   ...
   | HOW                 | WHAT                            |
   |---------------------+---------------------------------|
   | Class assignments   | Installing loading packages     |
   | [How do they work?] | .Rprofile configuration file    |
   |---------------------+---------------------------------|
...
** Objectives
   ...
   * [ ] Understand how "class assignments" work
...
** How do class assignments work?
   ...
   * Checking your phone during an assignment is not active
   * If you could not attend with a good excuse, submit later

** Next

   * Review of test 1

* Reviewing test 1 - w6s14 (02/14/22)
** Overview
...
** Test 1 results
   ...
   * Preparing such a test on paper and grading it while allowing for
     partial credit is a nightmare
   * Future tests will be online in Schoology

   #+caption: Test 1 DSC 205 results according to Schoology (11 Feb only)
   #+attr_html: :width 500px
   [[./img/test1_stats.png]]

   * [[test 1 results]] includes important base R code for graphics:
     - density distribution with ~density~ (smoothened histogram)
     - line plot with ~plot~
     - drawing a straight line with ~abline~
     #+name: test 1 results
     #+begin_src R :exports both :session :results output graphics file :file test1.png
       results <- c(8.33, 11.08, 11.91, 13.38, 13.67, 14, 14.16, 14.8, 16.16,
                    16.41, 16.75)
       mean(results)
       sd(results)
       ave <- mean(results)
...
** How should you study for and perform in a test?
